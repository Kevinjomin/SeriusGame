...
public class Grip
{
    ...
    public Grip(int width, int height)
    {
        _width = width;
        _height = height;
        _grip = new CellType[width, height];
    }
    ...
    public List<Point> GetWakableAdjacentCells(int x, int y, bool isAgent)
    {
        ...
        {
            if(IsCellWakable(_grip[adjacentCells[i].X, adjacentCells[i].Y], isAgent)==false)
            {
    ...
    public List<Point> GetAdjacentCellsOfType(int x, int y, CellType type)
    {
        ...
        for (int i = adjacentCells.Count - 1; i >= 0; i--)
        {
            if (_grip[adjacentCells[i].X, adjacentCells[i].Y] != type)
            {
                adjacentCells.RemoveAt(i);
            }
        }
    ...
    public CellType[] GetAllAdjacentCellTypes(int x, int y)
    {
        ...
        if (x > 0)
        {
            neighbours[0] = _grip[x - 1, y];
        }
        if (x < _width - 1)
        {
            neighbours[2] = _grip[x + 1, y];
        }
        if (y > 0)
        {
            neighbours[3] = _grip[x, y - 1];
        }
        if (y < _height - 1)
        {
            neighbours[1] = _grip[x, y + 1];
        }
        return neighbours;
    }
}